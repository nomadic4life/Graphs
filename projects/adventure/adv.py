...
# # # print("room exits", player.currentRoom.getExits())

# # room_hash = {}

# # def map_exits_to_rooms(current_id, exits, prev=None):
# #     mapped_rooms = {}

# #     for room_exit in exits.copy():
# #         print(room_exit, exits, prev)
# #         if prev is not None and room_exit in prev:
# #             mapped_rooms[room_exit] = prev[room_exit]
# #             exits.remove(room_exit)
# #         else:
# #             mapped_rooms[room_exit] = "?"

# #     if len(mapped_rooms) != 0:
# #         room_hash[current_id] = { "exits": mapped_rooms, "directions left": exits}
# #     else:
# #         print("okay what do I do from here, might not need this here?")

# # def move_player(room_id, direction="n"):
...
# #     if direction in room_hash[room_id]["exits"]:
# #         player.travel(direction, True)
# #     else:
# #         print("nothing in that direction")
# #     # print("test",room_hash[room_id]["exits"][direction])

# #     if direction in room_hash[room_id]["exits"] and room_hash[room_id]["exits"][direction] == "?":
# #         room_hash[room_id]["exits"][direction] = player.currentRoom.id
# #         room_hash[room_id]["directions left"].remove(direction)

# #     if player.currentRoom.id not in room_hash:
# #         map_exits_to_rooms(player.currentRoom.id, player.currentRoom.getExits(), prev)
# #     else:
# #         # do something if id is in room hash, maybe check directions left?
# #         #if any directions left go to next direciton in que?
# #         pass

# # #init, beginning of graph from room 0
# # # map_exits_to_rooms(player.currentRoom.id, player.currentRoom.getExits())

# # # move_player(player.currentRoom.id)
...
# #         move_player(player.currentRoom.id, cmds[0])
# #         print(room_hash)
# #         print("current room", player.currentRoom.id, room_hash[player.currentRoom.id])
# #         print("rooms left", 500 - len(room_hash))
...
#         """
#         self.vertices[room_id] = {"exits": {}, "room_exits": []}
# ​
...
#         mapped_exits = {}
#         reverse_direction = self.direction_opposite(direction)
#         room_exits = player.currentRoom.getExists()
#         for exit in room_exits.copy():
#             if exit not in self.vertices[current_room]["exits"]:
#                 if exit == reverse_direction:
#                     mapped_exits[exit] = prev_room
#                     room_exits.remove(exit)
#                 else:
#                     mapped_exits[exit] = "?"
#             else:
#                 mapped_exits[exit] = self.vertices[current_room]["exits"][exit]

#         self.vertices[current_room]["exits"] = map_exits
#         self.vertices[current_room]["room_exits"] = room_exits

#         self.vertices[prev_room]["exits"][direction] = current_room
#         self.vertices[prev_room]["room_exits"].remove(direction)
# ​
...
#                 map_exits[exit] = "?"
#         self.vertices[room_id]["exits"] = map_exits
#         self.vertices[room_id]["room_exits"] = room_exits
...
#         if current_room_id in self.vertices and len(self.vertices[current_room_id]["room_exits"]) == 0:
#             print("Deadend =(")
#         elif current_room_id in self.vertices:  # and dft?
...
#             self.vertices[prev_room_id]["room_exits"].remove(direction)
#             self.vertices[current_room_id]["room_exits"].remove(self.direction_opposite(
#                 direction))
#         elif current_room_id not in self.vertices:
#             self.add_vertex(current_room_id)
#             self.add_edge(current_room_id, prev_room_id, direction)

# ​
#     def dft_room(self):
#         room_exits = self.vertices[player.currentRoom.id]["room_exits"]
...
# ​
#         # if Deadend # BFS
#         if len(self.vertices[player.currentRoom.id]["room_exits"]) == 0 and len(self.vertices) != len(roomGraph):
#             print("Invoke BTS")
#             self.bts_deadend_room(player.currentRoom.id)
#         elif len(self.vertices) == len(roomGraph):
#             return True
# ​
...
class Graph:
    ...
    def add_edge(self, current_room, prev_room, direction):
        ...
        self.vertices[current_room]["exits"] = mapped_exits
        self.vertices[current_room]["room_exits"] = room_exits
    ...
    def move(self, direction, dft=True):
        ...
        if current_room_id in self.vertices and len(self.vertices[current_room_id]["room_exits"]) == 0:
            ...
            self.vertices[current_room_id]["exits"][self.direction_opposite(
                direction)] = prev_room_id
            print(direction, self.vertices[prev_room_id]["room_exits"])
            if direction in self.vertices[prev_room_id]["room_exits"]:
                self.vertices[prev_room_id]["room_exits"].remove(direction)
            if self.direction_opposite(direction) in self.vertices[current_room_id]["room_exits"]:
                self.vertices[current_room_id]["room_exits"].remove(
                    self.direction_opposite(direction))
        elif current_room_id not in self.vertices:
            self.add_vertex(current_room_id)
            self.add_edge(current_room_id, prev_room_id, direction)
    ...
    def dft_room(self):
        current_exits = self.vertices[player.currentRoom.id]["room_exits"]
        if len(self.vertices) >= len(roomGraph):
            return True
    ...
    def bts_deadend_room(self, starting_room, destination_vertex="?"):
        ...
        while q.size() > 0:
            ...
            if room not in visited:
                visited[room] = path
                if len(self.vertices[room]["room_exits"]) > 0:
                    path_direction = []
                    for room_id in visited[room]:
                        count += 1
                        if count == 10:
                            return
                        if room_id is not starting_room:
                            if "n" in self.vertices[room_id]["exits"]:
                                if self.vertices[room_id]["exits"]["n"] == prev:
                                    path_direction.append(
                                        "s")
                            if "s" in self.vertices[room_id]["exits"]:
                                if self.vertices[room_id]["exits"]["s"] == prev:
                                    path_direction.append(
                                        "n")
                            if "e" in self.vertices[room_id]["exits"]:
                                if self.vertices[room_id]["exits"]["e"] == prev:
                                    path_direction.append(
                                        "w")
                            if "w" in self.vertices[room_id]["exits"]:
                                if self.vertices[room_id]["exits"]["w"] == prev:
                                    path_direction.append(
                                        "e")
                            # for direction in self.vertices[room_id]["exits"]:
                            #     if self.direction_opposite(direction) in self.vertices[room_id]["exits"] and self.vertices[room_id]["exits"][self.direction_opposite(direction)] == prev:
                            #         path_direction.append(
                            #             direction)
                            #         break
                            #     else:
                ...
                for direction in self.vertices[room]["exits"]:
                    path_copy = list(path)
                    print(self.vertices[room]["exits"][direction])
                    if self.vertices[room]["exits"][direction] not in visited:
                        path_copy.append(
                            self.vertices[room]["exits"][direction])
                        q.enqueue(path_copy)
