...
# -------------------------------- #
# visited room = tuple("coords" = tuple(x, y),"room_id" = num)

# create a node
def create_node(direction=None, visited=None):
    if visited is None:
        visited = {}
    room_id = player.currentRoom.id
    exits = player.currentRoom.getExits()
    size = len(exits)
    coords = player.currentRoom.getCoords()
    exits = map_exits(exits)
    neighbors = map_neighbors(exits)
    ...
    node = {
        ...
        "exits": exits,
        "neighbors": neighbors,
        "entered_from": reverse(direction),
        "is_deadend": check_is_deadend(size),
        "is_hub": check_is_hub(size)
    }

    if room_id not in visited:
        visited[room_id] = node

    if node["is_deadend"]:
        print("room id", node["room_id"], "is a deadend")
    # print(node)
    
    return node

def check_is_deadend(size):
    if size == 1:
        return True
    else:
        return False

def check_is_hub(size):
    if size > 2:
        return True
    else:
        return False

# maybe also BFT in step in all exits?
def map_exits(exits):
    mapped = {}

    for e in exits:
        player.travel(e)
        mapped[e] = {"coords": player.currentRoom.getCoords(), "room_id": player.currentRoom.id}
        player.travel(reverse(e))
    
    return mapped

def map_neighbors(exits):
    arr = []
    for e in exits:
        direction = {"direction": e, "room_id": exits[e]["room_id"]}
        arr.append(direction)
    arr = sorted(arr, key = lambda i: i["room_id"], reverse=True)
    return arr
# return reverse direction
def reverse(direction=None):
...
# quick longest path from 0
def move_fast(node, visited=None):
    if visited is None:
        visited = {}
    ...
    for direction in exits:
        # print("go", direction, exits[direction] )
        if exits[direction]["room_id"] < node["room_id"] and direction != node["entered_from"]:
            # print(min_id)
            if min_id is None:
                min_id = exits[direction]["room_id"]
            elif exits[direction]["room_id"] < min_id:
                min_id = exits[direction]["room_id"]
            travel_down = direction
        else:
            if exits[direction]["room_id"] not in visited:
                # print(max_id)
                if max_id is None:
                    max_id = exits[direction]["room_id"]
                    move = direction
                elif exits[direction]["room_id"] > max_id:
                    max_id = exits[direction]["room_id"]
                    move = direction

    print("-------------->", travel_down, move, min_id, max_id)
    if travel_down:
        move = travel_down


    player.travel(move)
    print("move to room", player.currentRoom.id)

    return move

def move_back_to_parent_hub(node):
    # print("top here ->>>>>", node)
    while not node["is_hub"]:
        move = node["entered_from"]
        player.travel(move)
        room_id = player.currentRoom.id
        print("move back to", room_id)
        node = visited[room_id]

    travel_back = True
    for e in node["exits"]:
        if node["exits"][e]["room_id"] not in visited:
            travel_back = False

    print(travel_back)
    
    if travel_back:
        # print(visited[node["exits"][node["entered_from"]]["room_id"]]["entered_from"])
        # move = visited[node["exits"][node["entered_from"]]["room_id"]]["entered_from"]
        move = node["entered_from"]
        player.travel(move)
        room_id = player.currentRoom.id
        print("move back to", room_id)
        node = visited[room_id]
        node = move_back_to_parent_hub(node)

    # print("return node", node)
    return node

def all_neigbor_rooms_visited(node):
    for e in node["exits"]:
        if node["exits"][e]["room_id"] not in visited:
            return False
    return True
...
#     exits = node["exits"]
#     for e in exits:
#         if exits[e]["room_id"] not in visited:
#             # print(max_id)
#             if max_id is None:
#                 max_id = exits[direction]["room_id"]
#                 move = direction
...
#     is_move_up = True
#     # path = [node]
#     # memo = {node["room_id"]: path.copy()}
#     # longest_path_end_room_id = node["room_id"]

#     loop = 0
#     counter = 0
#     while search_stack.size() > 0 and counter < 190:
#         # print(counter)
#         # print("search", search_stack.stack)
#         # print("path", path)
#         counter += 1
#         # if not is_hub:
#         #     node = search_stack.pop()

#         node = search_stack.stack[-1]

#         # print("room id", node["room_id"])

#         if node["room_id"] == loop:
#             if all_neigbor_rooms_visited(node):
#                 search_stack.pop()
#                 node = search_stack.stack[-1]

                
#         if not node["is_deadend"]:
#             # if node["room_id"] == loop:

#             move = move_fast(node, visited=visited)
#             node = create_node(move, visited=visited)
#             search_stack.push(node)
#             # path.append(node)
#         if node["is_deadend"]:
#             node = search_stack.pop()
#             print(node)
#             node = move_back_to_parent_hub(node)
#             while search_stack.stack[-1]["room_id"] != node["room_id"]:
#                 print("is it removing from stack?")
#                 search_stack.pop()
#     print(search_stack.stack)
...
def move_to_neighbor(node):
    for neighbor in node["neighbors"]:
        if neighbor["room_id"] not in visited:
            move = neighbor["direction"]
            break
        elif neighbor["room_id"] < node["room_id"]:
            move = neighbor["direction"]
            break
        # elif()
    if node["room_id"] == 0:
        print("in room 0. all neigbors visited, no where to go")
    player.travel(move)
    return move

def check_neigbors(node):
    completed = True
    for neighbor in node["neighbors"]:
        if neighbor["room_id"] not in visited:
            completed = False
...
def move_to_prev_neighbor(node):
    move = node["entered_from"]
    player.travel(move)
    node = visited[player.currentRoom.id]
    return node 
...
travel = Stack()
travel.push(node)
while count < 1000: # 934
    count += 1
    move = move_to_neighbor(node)
    traversalPath.append(move)
    node = create_node(move, visited=visited)
    travel.push(node)
    if node["room_id"] == 0:
        if check_neigbors(node):
            break

while travel.size() > 0 and count < 1500 and len(visited) != 500:
    count += 1
    # node = travel.pop()
    if not check_neigbors(node):
        move = move_to_neighbor(node)
        traversalPath.append(move)
        node = create_node(move, visited=visited)
        travel.push(node)
        ...
        move = travel.pop()
        node = move_to_prev_neighbor(move)
        traversalPath.append(move["entered_from"])


while node["room_id"] != 0:
    node = move_to_prev_neighbor(node)
    node = visited[node["room_id"]]
...
# def travel_stack_que(node):
#     search_stack = Stack()
#     search_que = Queue()
#     path = [node]

#     move = move_up(node)
#     node = create_node(move)
#     path.append(node)

#     if node["is_hub"]:
#         #check neigbors visited


#     is_deadend = node["is_deadend"]
#     if is_deadend:


# algo for hub
# check for largest neigbor in hub
    # build array of neigbors with largest value first
# loop through arry of neigbors,
    # if neighbor not visited
        # move to neigbor
        # break loop
    # elif neigbor id < room id:
        # move to neighbor

#  algo for non hub
    # loop through array of neighbors:
        # if neighbor not visiited:
            # move to neigbor
            # break loop
        # elif neighbor id < room id:
            # move to neighbor
...
# check if 0 is hub
    # add id 0 to que hub
# check for largest number which is 8
    # check if 8 has been visited it has not
        # move to 8

# add 8 to path
# check if 8 is hub 
    # it is not we don't had 8 to que hub
# check if 8 is deadend; 
    # it is not move to next biggest avialble id
# check for largest number which is 16
    # check if 16 has been visited, it has not
        # move to 16

# add 16 to path
# check if 16 is hub
     # it is not we don't had 16 to que hub
# check if 16 is dead end
    # 16 is deadend
    # path is [0, 8, 16]
    # copy path new_path = path.copy()
    # update memo 16 : [0, 8 16]
    # return_to = search_que.dequeue() 
    # return_to node id 0
    # move = path.pop() = 16
    # move_back to prev id which is 8
    # check if prev id (8) == return_to (0):
        # it is not move_back to prev id which is 0
    # chek if prev id (0) == return_to (0)
...
# while travel.size() > 0 and count < 1400 and len(visited) != 500:
#     count += 1
#     # node = travel.pop()
#     if not check_neigbors(node):
#         move = move_to_neighbor(node)
...
# print("printing node ===>>>>>>>>>\n\n",node ,len(visited), count)
# print( len(travel.stack))
# print(move)

# node = create_node(visited=visited)
# dft(node, visited=visited)
# move = move_fast(node, visited=visited)
# node = create_node(move, visited=visited)
# move = move_fast(node, visited=visited)
# node = create_node(move, visited=visited)
...
# node = create_node(move, visited=visited)
# print(visited)
# print(move_fast(node, visited=visited))
# player.travel("n")
# print(create_node("n"))